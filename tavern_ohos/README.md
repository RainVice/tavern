...
`tavern_ohos` 是一个面向 OpenHarmony / HarmonyOS 应用的无 UI SillyTavern 后台运行时 SDK。它不包含 Web UI，也不打包 Node.js SillyTavern Server；宿主应用负责界面、网络传输、权限申请和平台能力接入，本 SDK 负责角色、聊天、消息、Prompt、世界书、预设、插件兼容子集、资源文件、RAG、流式响应解析、导入导出和事件分发等后台逻辑。
...
SDK 当前提供的是 headless runtime，不直接负责 UI 渲染，也不直接持有真实 OpenAI HTTP 发送实现。Provider 相关模块会构造请求体、管理流式任务、解析 OpenAI-compatible SSE chunk；真实网络请求需要宿主应用通过 HarmonyOS `http`、WebSocket、SSE 或自有网络层实现后接入 `ProviderStreamSource`。
...
- 真实持久化文件系统适配，如果不使用内存存储。
- OpenAI、OpenRouter、Ollama、Whisper、TTS、Embedding 等真实 HTTP 请求。
...
`InMemoryFileStore` 适合单元测试和临时会话。正式应用应实现 `TavernFileStore`，把 `readText`、`writeText`、`exists`、`delete`、`list`、`ensureDirectory` 等操作映射到应用沙箱文件系统。

## 存储适配

大部分本地数据 Runtime 都接收 `TavernFileStore`，并在 `rootPath/users/<userId>/...` 下组织数据。内置 `InMemoryFileStore` 可直接用于测试：

```ts
import { InMemoryFileStore, type TavernFileStore } from 'tavern_ohos';
...
注意：当前 `NetworkService` 对 `openrouter`、`generic` 等部分 provider 会自动补 `Authorization`；对 `openai` / `openai-compatible` 建议先在 `additionalHeaders` 中显式配置 `Authorization`，或在宿主网络层自行加上 `Bearer`。
...
流式模型调用需要宿主实现 `ProviderStreamSource`。SDK 只要求这个 source 把服务端返回的 SSE 文本 chunk 交给 `ProviderStreamSink`。
...
class HttpSseSource implements ProviderStreamSource {
  async start(request: ProviderStreamRequest, sink: ProviderStreamSink): Promise<ProviderStreamHandle> {
    // 这里由宿主接入 HarmonyOS http / WebSocket / SSE 客户端。
    // 每收到一段原始 SSE 文本，就调用 await sink.onChunk(chunk)。
...
### 为什么 Provider 不直接发 OpenAI 请求？

OpenHarmony 网络、权限、证书、代理、后台任务和系统 API 通常由宿主应用统一治理。SDK 目前只定义请求结构、连接配置、流式解析和事件流，真实 HTTP 层由宿主接入更安全，也更容易适配不同产品策略。

### `openai` 请求头会自动加 Authorization 吗？

当前建议宿主在 `additionalHeaders` 或网络层显式加 `Authorization: Bearer <key>`。`NetworkService` 已对部分 provider 自动补 Bearer，但 `openai` / `openai-compatible` 分支需要宿主侧明确处理或后续补齐。
